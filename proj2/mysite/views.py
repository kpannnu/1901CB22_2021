...
def index(request):
    if request.method=='POST':
        uploaded_file4=request.FILES['roll']
        fs=FileSystemStorage()
        fs.save(uploaded_file4.name,uploaded_file4)
        uploaded_file5=request.FILES['subjects']
        fs=FileSystemStorage()
        fs.save(uploaded_file5.name,uploaded_file5)
        uploaded_file3=request.FILES['myfile1']
        fs=FileSystemStorage()
...
def roll_subjects(request):
    if request.method=='POST':
        uploaded_file1=request.FILES['seal']
        fs=FileSystemStorage()
        fs.save(uploaded_file1.name,uploaded_file1)
        uploaded_file2=request.FILES['signature']
        fs=FileSystemStorage()
...
class PDF(FPDF):  # class made to use fpdf to make pdf
    ...
    def seal(self, x1, y1, w1, h1):  # function to add seal on pdf if asked by the user
        if os.path.exists('Input_files/seal.JPG'):
            self.image(name=os.getcwd()+r"/Input_files/seal.JPG", x=x1, y=y1, w=w1, h=h1)

    def signature(self, x1, y1, w1, h1):  # function to add signature on pdf if asked by the user
        if os.path.exists('Input_files/Signature.JPG'):
            self.image(name=os.getcwd()+r"/Input_files/Signature.JPG", x=x1, y=y1, w=w1, h=h1)
    ...
    for x in dict1:    # iterating dictionary of grades.csv file
        ...
        if(dict1[x][0] != roll or x == len(dict1)):
            if(count != 0):
                ...
                if os.path.exists('Input_files/seal.JPG'):  # checking if seal is required by user
                    pdf.seal(180, 250, 30, 30)

                if os.path.exists('Input_files/Signature.JPG'):  # checking if signature is required by user
                ...
                pdf.output('transcriptsIITP/{}.pdf'.format(roll), 'F')
...
def range1(request):
    if request.method=='POST':
        ...
        for aa in range(b2-a2+1):
            c = str(a)+str(a1)
            d=s1[0]+s1[1]+s1[2]+s1[3]+s1[4].upper()+s1[5].upper()+c
            x=dict3.get(d,0)
            a2 = a2+1
            a1 = a2 % 10
            a = a2//10
        ...
        for aaa in l:
            fun(aaa)
    return render(request,'index.html',{'message':"Roll numbers that doesn't exist are: ",'message1':l1})    
...
def downloadfile(request):  
    zip_directory('C:\\Users\\Abhijeet\\Desktop\\Project-2\\transcriptsIITP', 'C:\\Users\\Abhijeet\\Desktop\\Project-2\\transcriptsIITP.zip')
    base_dir=os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    filename='/transcriptsIITP.zip'
